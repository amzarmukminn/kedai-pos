...
(() => {
  ...
  window.completeTransaction = async function () {
    ...
    if (appState.cart.some(item => item.temp)) return alert('Item harga manual belum disokong dalam sistem berpusat. Tambah produk dahulu supaya stok dan audit kekal tepat.');
    const total = getCartTotal();
    const paid = getPaidAmount();
    const method = document.getElementById('paymentMethod').value;
    if (method === 'cash' && paid < total) return alert('Amaun tunai tidak mencukupi.');
    if (method !== 'cash' && Math.abs(paid - total) > 0.009) return alert('Untuk Kad atau DuitNow, masukkan jumlah tepat RM ' + total.toFixed(2) + '.');
    const button = document.getElementById('completeBtn'); button.disabled = true;
    const { data, error } = await client.rpc('complete_sale', {
      p_items: appState.cart.map(item => ({ product_id: item.productId, quantity: item.quantity })),
      p_method: method,
      p_cash_received: method === 'cash' ? paid : null,
